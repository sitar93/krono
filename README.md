...
| **`AGENTS.md`** | Developers and AI agents: build, architecture, extending the code |
...
4. **Calculation swap:** Short **Mode** press, or rising gate on **PB4** (when not in mode-change UI). Swaps A/B mapping for the current mode. Aux LED blinks.
...
- **Option A — Save only:** Do **not** press Mode within 5 s. Current state (tempo, mode, per-mode swap, mode-specific parameters such as swing/chaos/binary bank) is **written to Flash**. Aux blinks once; Status LED returns to normal blinking for the current mode. This is the **primary save path**.
...
- **Euclidean / Musical / Probabilistic / Sequential / Swing / Polyrhythm / Phasing / Chaos / Binary:** See the table below; **Swap** inverts or exchanges A/B data sets as described per mode.
- **Swing:** Additional profile selection via the mode-change flow while Swing is active (see `variables.h` / firmware).
- **Binary:** **MOD** or **PB4** cycles **banks** 0–9; transition can be aligned to pattern boundaries in firmware.

---

## Operational modes (reference)

Mode order and **status LED blink count** (1 = Default … 11 = Binary):
...
| 11 | **BINARY** | 16-step binary patterns at **4×** main clock; drum-style mapping on 2–6; **10 banks** (0–9), **MOD** or **PB4** advances bank; banks persisted. |

---

## Project structure (firmware)

- **`src/main.c`** — Init, main loop, callbacks, aux LED timer, `millis()`; load/save wiring for chaos divisor, swing profiles, binary bank.
- **`src/input_handler.c`** — Pin init, op-mode state machine, tap-interval averaging (`NUM_INTERVALS_FOR_AVG`), external clock handoff, tempo callback dispatch, calc/binary actions.
...
- **`krono_state_t`** in Flash (magic + checksum). Save is requested through the **mode-change UI timeout** path (see Quick start). Cooldown: `SAVE_STATE_COOLDOWN_MS` in `variables.h`.
...
- **Calc swap:** short MOD or PB4 gate (when idle path allows); binary mode may use MOD for **bank** instead of generic calc swap—see mode behavior above.
