...
void clock_manager_set_internal_tempo(uint32_t interval_ms) {
    if (interval_ms > 0 && interval_ms != active_tempo_interval_ms /*&& ext_clock_has_timed_out(millis())*/) {
        active_tempo_interval_ms = interval_ms;
        // Optional: Sync internal timer immediately?
        // last_f1_pulse_time_ms = millis(); // Reset timer phase
        // f1_tick_counter = 0;
    }
}
