...
static void on_tap_tempo_change(uint32_t new_interval_ms);
...
static void on_tap_tempo_change(uint32_t new_interval_ms) {
	if (new_interval_ms > 0) {
		clock_manager_set_internal_tempo(new_interval_ms);
...
static void on_op_mode_change(uint8_t mode_clicks) {
	...
	if (mode_clicks > 0 && mode_clicks <= NUM_OPERATIONAL_MODES) {
        ...
        bool mode_is_changing = (desired_mode != g_current_op_mode);
