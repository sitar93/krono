...
- **Build:** `platformio run -e blackpill_f401ce`
- **Upload:** `platformio run -e blackpill_f401ce --target upload`
  ...
  - The `upload_protocol = dfu` and `board = blackpill_f401ce` settings are configured in `platformio.ini`.
- **Clean:** `platformio run -e blackpill_f401ce --target clean`
...
- Always run `platformio run -e blackpill_f401ce` after edits to check for compilation errors. Verify functionality by uploading and observing hardware behavior.
